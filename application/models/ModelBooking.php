...
class ModelBooking extends CI_Model
{
    ...
    public function joinOrder($where)
    {
        ...
        $this->db->join('service bu ', 'bu.id=d.id_service');
        $this->db->where($where);
        return $this->db->get();
    }
    public function simpanDetail($where = null)
    {
        $sql = "INSERT INTO booking_detail (id_booking,id_service) SELECT booking.id_booking,temp.id_service FROM booking, temp WHERE temp.id_user=booking.id_user AND booking.id_user='$where'";
        $this->db->query($sql);
    }
    ...
    public function find($where)
    {
        ...
        $this->db->limit(1);
        return $this->db->get('service', $where);
    }
    ...
    {
        $this->db->query('CREATE TABLE IF NOT EXISTS temp(id_booking varchar(12), tgl_booking DATETIME, email_user varchar(128), id_service int)');
    }
    public function selectJoin()
    {
        ...
        $this->db->join('service', 'booking_detail.id_service=service.id');
        return $this->db->get();
    }
